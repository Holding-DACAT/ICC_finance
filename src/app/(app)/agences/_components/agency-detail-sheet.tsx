      ...
      <SheetContent>
        {agency ? (
          <>
            ...
            <SectionTitle icon={ShieldCheck}>Conformité</SectionTitle>
            <div className="grid grid-cols-2 gap-2.5">
              <Info label="RC Pro — assureur" value={agency.rcProInsurer ?? "—"} />
              <Info label="RC Pro — échéance" value={formatDate(agency.rcProExpiry)} />
              <Info
                label="Garantie financière"
                value={agency.guaranteeAmount != null ? formatEur(agency.guaranteeAmount) : "—"}
              />
              <Info label="Garantie — échéance" value={formatDate(agency.guaranteeExpiry)} />
            </div>
